...
drv_ctrl_t *init_console_driver()
{
	...
	map_page((pte *)pt_root, drv_console_start,
		 drv_console_start - EDRV_VA_PA_OFFSET, n_console_drv_pages,
		 PTE_V | PTE_X | PTE_W | PTE_R);

	cmd_handler console_handler = (cmd_handler)drv_console_start;

	drv_ctrl_t *console_ctrl =
		(drv_ctrl_t *)console_handler(QUERY_INFO, 0, 0, 0);
	printd("%x\n", console_ctrl->reg_addr);

	uintptr_t console_va = ioremap((pte *)pt_root, console_ctrl->reg_addr,
				       console_ctrl->reg_size);
	printd("%x\n", console_va);
