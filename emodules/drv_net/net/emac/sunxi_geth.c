...
static int geth_sys_init(void)
{
	...
	char *phy_mode = NULL;
	int geth_nodeoffset;
	unsigned char __maybe_unused i;
	u32 tx_delay = 0;
	u32 rx_delay = 0;
	u32 use_ephy_clk = 0;
...
/* it should be defined in uboot/include/configs/sun?iw?p?.h */
	...
	printf("PHY_CLK_REG_REMAPPED value: 0x%x\n", value);
...
int geth_initialize(bd_t *bis)
{
	...
	dev = (struct eth_device *)my_malloc(sizeof *dev);
	if (!dev)
		return -12;
