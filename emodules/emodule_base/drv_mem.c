...
void init_mem(uintptr_t id, uintptr_t mem_start, uintptr_t usr_size,
	      drv_addr_t drv_list[MAX_DRV], uintptr_t argc, uintptr_t argv)
{
	printd("[init_mem] %s %x\n", argv, argc);
   ...
   * payload */
	...
	int cnt = 0;
	printd("[init_mem] drv_list[0].drv_start = 0x%x\n",
	       drv_list[0].drv_start);
	while (drv_list[cnt].drv_start) {
		printd("[init_mem] drv_list[%d].drv_start: 0x%x\n", cnt,
	...
	if (drv_list != 0) {
		...
		uintptr_t drv_pa_start =
			PAGE_DOWN(drv_list[0].drv_start - EDRV_VA_PA_OFFSET);
		uintptr_t drv_pa_end =
			PAGE_UP((uintptr_t)drv_list + 64 * sizeof(drv_addr_t));
		printd("[init_mem] drv_pa_end = 0x%x drv_pa_start = 0x%x\n",
		       drv_pa_end, drv_pa_start);
		map_page((pte *)pt_root, PAGE_DOWN(drv_list[0].drv_start),
			 drv_pa_start,
			 (drv_pa_end - drv_pa_start) >> EPAGE_SHIFT,
			 PTE_V | PTE_R | PTE_X);
	}
	/* base driver remaining mem */
	/* thus easier manupilating satp */
	map_page((pte *)pt_root, EDRV_VA_PA_OFFSET + usr_avail_start,
		 usr_avail_start, PAGE_DOWN(usr_avail_size) >> EPAGE_SHIFT,
		 PTE_V | PTE_W | PTE_R);
	...
	map_page((pte *)pt_root, EDRV_VA_PA_OFFSET + init_data_start,
		 init_data_start, n_base_init_data_pages,
		 PTE_V | PTE_W | PTE_R);
	...
	alloc_page((pte *)pt_root, drv_sp, n_base_stack_pages,
		   PTE_V | PTE_W | PTE_R, DRV);
