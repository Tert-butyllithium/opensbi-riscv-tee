...
int md2_update(md2ctx_t *ctx, const void *data, size_t size)
{
	...
	if (ctx->last.used_cnt != 0) {
		if (ctx->last.used_cnt + size < BLK_SIZE) {
			md2_memcpy(&ctx->last.buf[ctx->last.used_cnt], data,
				   size);
			ctx->last.used_cnt += size;
			return MD2_OK;
		} else {
			copied_len = BLK_SIZE - ctx->last.used_cnt;
			md2_memcpy(&ctx->last.buf[ctx->last.used_cnt], data,
				   copied_len);
...
int md2_final(void *md, md2ctx_t *ctx)
{
	...
	padding_len = BLK_SIZE - ctx->last.used_cnt;
	...
	md2_memcpy(ctx->last.buf, ctx->checksum, BLK_SIZE);
	ret_state = md2_process_block(ctx, ctx->last.buf);
	if (ret_state != MD2_OK) {
		return ret_state;
	}
	md2_memcpy(md, ctx->hash_buf, BLK_SIZE);
	return MD2_OK;
}
